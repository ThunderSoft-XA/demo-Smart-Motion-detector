...
{
    gizLog(LOG_INFO,"[uploadDataCBFunc]: result %d \r\r\n", result);
}
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    {
        gizLog(LOG_WARNING,"gizDataPoint2Event Error , Illegal Param\r\r\n");
        return -1;
    }
    ...
    if(sizeof(issuedData->attrFlags) > 1)
    {
        ...
        {
            gizLog(LOG_WARNING,"gizByteOrderExchange Error\r\r\n");
            return -1;
        }
    }
    
    if(0x01 == issuedData->attrFlags.flagLedValue)
    {
        info->event[info->num] = EVENT_LedValue;
        info->num++;
        dataPoints->valueLedValue = gizStandardDecompressionValue(LedValue_BYTEOFFSET,LedValue_BITOFFSET,LedValue_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagMotorValue)
    {
        info->event[info->num] = EVENT_MotorValue;
        info->num++;
        dataPoints->valueMotorValue = gizStandardDecompressionValue(MotorValue_BYTEOFFSET,MotorValue_BITOFFSET,MotorValue_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        gizLog(LOG_WARNING,"gizCheckReport Error , Illegal Param\r\r\n");
        return -1;
    }
    currentTime = gizGetTimerCount();
    if(last->valueLedValue != cur->valueLedValue)
    {
        gizLog(LOG_INFO,"valueLedValue Changed\r\r\n");
        ret = 1;
    }
    if(last->valueMotorValue != cur->valueMotorValue)
    {
        gizLog(LOG_INFO,"valueMotor Changed\r\r\n");
        ret = 1;
    }
    ...
    if(last->valueHeartRateValue != cur->valueHeartRateValue)
    {
        ...
        {
            gizLog(LOG_INFO, "valueHeartRateValue Changed\r\r\n");
            ret = 1;
        }
    }
    if(last->valueX_axis_Value != cur->valueX_axis_Value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO, "valueX_axis_Value Changed\r\r\n");
            ret = 1;
        }
    }
    if(last->valueY_axis_Value != cur->valueY_axis_Value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO, "valueY_axis_Value Changed\r\r\n");
            ret = 1;
        }
    }
    if(last->valueZ_axis_Value != cur->valueZ_axis_Value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO, "valueZ_axis_Value Changed\r\r\n");
            ret = 1;
        }
    }
    if(last->valuePressure_Value != cur->valuePressure_Value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO, "valuePressure_Value Changed\r\r\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    {
        gizLog(LOG_WARNING,"gizDataPoints2ReportData Error , Illegal Param\r\r\n");
        return -1;
    }
    ...
    if(0 != value)
    {
        ...
        else
        {
            ...
            if(1 == status.types.station)
            {
                ...
                gizwitsProtocol.issuedProcessEvent.num++;
                gizLog(LOG_INFO,"OnBoarding: Station mode\r\r\n");
            }
        }
...
int32_t ICACHE_FLASH_ATTR gizIssuedProcess(uint8_t *didPtr, uint8_t *inData, uint32_t inLen,uint8_t *outData,int32_t *outLen)
{
    ...
    {
        gizLog(LOG_WARNING,"!!! IssuedProcess Error \r\r\n"); 
        return -1;
    }

    if(NULL == didPtr)
    {
        gizLog(LOG_INFO,"~~~gizIssuedProcess: did is NULL .\r\r\n");
    }
    else
    {
        gizLog(LOG_INFO,"~~~gizIssuedProcess: did %s\r\r\n", didPtr);
    }
        ...
        switch(inData[0]) 
        {
            ...
            case ACTION_READ_DEV_STATUS:
                ...
                *outLen = sizeof(gizwitsReport_t);
                ...
                gizLog(LOG_INFO,"%s: ", "~~~ReadReport \r\r\n");
    ...
    { 
        gizLog(LOG_WARNING," Error : didPtr  \r\r\n");
    }
...
void ICACHE_FLASH_ATTR gizwitsTask(ULONG para)
{
    ...
    gizLog(LOG_INFO, "[gizwitsTask] Gizwits Task Start ...\r\r\n");
    ...
    while(1)
    {
        ...
        {
            gizLog(LOG_INFO,"[gizwitsTask] Receive Message %d \r\r\n", gizTaskComm.message);
            switch (gizTaskComm.message)
            {
                case SIG_IMM_REPORT:
                    gizLog(LOG_INFO,"[SIG] :SIG_IMM_REPORT \r\r\n");
                 ...
                 case SIG_UPGRADE_DATA:
                    gizLog(LOG_INFO,"[SIG] :SIG_UPGRADE_DATA\r\r\n"); 
                    gizwitsHandle((dataPoint_t *)&currentDataPoint);
                 break;
                 deafult:
                    gizLog(LOG_WARNING,"[SIG] NULL \r\r\n"); 
                 break;
            }
        }
    }
...
int8_t ICACHE_FLASH_ATTR gizwitsHandle(dataPoint_t *dataPoint)
{
    if(NULL == dataPoint)
    {
        gizLog(LOG_WARNING,"!!! gizReportData Error \r\r\n");
...
void gizSendQueue(uint8_t message)
{
    ...
    if (TX_SUCCESS != status)
    {
        gizLog(LOG_WARNING,"[gizSendQueue] Send Message Failed [%d]\r\r\n",status);
    }   
}
/**
...
void ICACHE_FLASH_ATTR gizTimerFunc(void)
{
    ...
    if(0 == repCtime%10)
    {
        ...
        gizLog(LOG_INFO,"[gizTimerFunc] One Second \r\r\n", gizwitsProtocol.timerMsCount);
        if(0 == gizwitsProtocol.timerMsCount%600)
        {
            gizLog(LOG_INFO,"[gizTimerFunc] 10 Minute Report ...\r\r\n");
...
void ICACHE_FLASH_ATTR gizwitsInit(void)
{
    ...
    gizLog(LOG_INFO,"[gizwitsInit] Init Begin \r\r\n");
...
//Queue Creat
    ...
    {
        gizLog(LOG_WARNING,"[gizwitsInit] txm_module_object_allocate [byte_pool_task] failed, %d \r\r\n", ret_txm);
        return;
    }
    ...
    if(ret_txm != TX_SUCCESS)
    {
        gizLog(LOG_WARNING,"[gizwitsInit] tx_byte_pool_create [byte_pool_task] failed, %d \r\r\n", ret_txm);
        return;
    }
    ...
    if(ret_txm != TX_SUCCESS)
    {
        gizLog(LOG_WARNING,"[gizwitsInit] txm_module_object_allocate gizQueue failed, %d \r\r\n", ret_txm);
        return;
    }
    ...
    if (TX_SUCCESS != status)
    {
        gizLog(LOG_WARNING,"[gizwitsInit] gizQueue Create Failed [%d] \r\r\n",status);
        return;
    }
    else
    {
        gizLog(LOG_WARNING,"[gizwitsInit] gizQueue Create Success [%d] \r\r\n",status);
    } 

//Task Creat
    if(TX_SUCCESS != txm_module_object_allocate((VOID *)&gizTaskHandle, sizeof(TX_THREAD)))
    {
        gizLog(LOG_WARNING,"[gizwitsInit] gizTaskHandle malloc failed \r\r\n");
        return;
    }
    ...
    if(ret != TX_SUCCESS)
    {
        gizLog(LOG_INFO,"[gizwitsInit] : Thread gizwitsTask Create Failed [%d]\r\r\n",ret);
        return;
    }
    else
    {
        gizLog(LOG_INFO,"[gizwitsInit] Thread gizwitsTask Create Success [%d]\r\r\n",ret);        
    }
    ...
    // if(ret != TX_SUCCESS)
    // {
    //     gizLog(LOG_WARNING,"[gizwitsInit] gizTimer Create Failed [%d]\r\r\n",ret);
    // }
    // else
    // {
    //     gizLog(LOG_INFO,"[gizwitsInit] gizTimer Create Success [%d]\r\r\n",ret);
    // }
    ...
    gizLog(LOG_INFO,"[gizwitsInit] gizTimer Create [%d]\r\r\n",qStatus);
    ...
    gizLog(LOG_INFO,"gizwitsInit OK \r\r\n");
}
...
int32_t ICACHE_FLASH_ATTR gizwitsPassthroughData(uint8_t * data, uint32_t len)
{
    ...
    if(NULL == data)
    {
        gizLog(LOG_INFO,"!!! gizReportData Error \r\r\n");
    ...
    {
        gizLog(LOG_INFO,"Failed to malloc buffer\r\r\n");
        return (-1);
    }
