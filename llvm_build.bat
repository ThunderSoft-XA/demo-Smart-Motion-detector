...
rem set TOOLCHAIN_DIR=D:\software\clang
rem set PYTHON_DIR=D:\software\Python27
...
set SOC_DRIVER_ADXL345=%PROJECT_PATH%\driver\adxl345
set SOC_DRIVER_SPI=%PROJECT_PATH%\driver\spi
...
for %%x in (%SOC_DRIVER_ADXL345%\*.c) do (
%COMPILER%  -c  %DAM_CPPFLAGS% %GAGENT_CPPFLAGS% %DAM_CFLAGS% %APP_CFLAGS% %DAM_INCPATHS% %GAGENT_INC% %SOC_DRIVER_ADXL345% %%x 
 if errorlevel 1 (goto :eof)
)
for %%x in (%SOC_DRIVER_SPI%\*.c) do (
%COMPILER%  -c  %DAM_CPPFLAGS% %GAGENT_CPPFLAGS% %DAM_CFLAGS% %APP_CFLAGS% %DAM_INCPATHS% %GAGENT_INC% %SOC_DRIVER_SPI% %%x 
 if errorlevel 1 (goto :eof)
)
